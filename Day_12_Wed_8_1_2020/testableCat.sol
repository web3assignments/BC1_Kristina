...
contract SchrodingersCat {
    ...
    */
    ...
    function getCatState(uint boxId) public view returns(string memory catstate){
        return (boxes[msg.sender][boxId]).cat_state == CatState.Superposition ? "Superposition" : ((boxes[msg.sender][boxId]).cat_state == CatState.Alive ? "Alive" : "Dead");
    }


    function getBoxState(uint boxId) public view returns(string memory boxstate){
        return (boxes[msg.sender][boxId]).box_state == BoxState.Open ? "Open" : "Closed";
    }
